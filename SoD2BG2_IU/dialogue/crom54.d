EXTEND_BOTTOM WSMITH01 13
  IF ~PartyHasItem("ax1h12")~ THEN GOTO NewItem //Stonefire +5
END
  
APPEND WSMITH01
  IF ~~ THEN BEGIN NewItem SAY @193
    IF ~!PartyHasItem("dtkruby")~ THEN GOTO NewItem2
    IF ~PartyHasItem("dtkruby")~ THEN GOTO NewItem3
  END

  IF ~~ THEN BEGIN NewItem2 SAY @194
    IF ~~ THEN GOTO NoThanks
  END
  
  IF ~~ THEN BEGIN NewItem3 SAY @195
    IF ~PartyGoldLT(10000)~ THEN REPLY #67011 GOTO NoThanks
    IF ~PartyGoldGT(9999)~ THEN REPLY #67012 DO ~SetGlobal("DTKItems","ar0334",66)
                                                 SetGlobal("ForgeStuff","GLOBAL",1)
                                                 TakePartyGold(10000)
                                                 TakePartyItemNum("ax1h12",1)
                                                 TakePartyItemNum("dtkruby",1)
                                                 DestroyItem("ax1h12")
                                                 DestroyItem("dtkruby")
                                                 DestroyGold(10000)~ GOTO 56
    IF ~~ THEN REPLY #67013 GOTO NoThanks
  END
  
  IF ~~ THEN BEGIN NoThanks SAY @3
   COPY_TRANS WSMITH01 13
  END
END